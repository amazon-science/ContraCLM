...
from transformers import (CodeGenModel, CodeGenTokenizer, RobertaModel,
                          RobertaTokenizer)
...
def convert_examples_to_features(js, tokenizer, args, lang):
    ...
    if isinstance(tokenizer, RobertaTokenizer):
        ...
        if args.model_type == 'hf_unixcoder':
            code_tokens = tokenizer.tokenize(code)[:args.code_length - 4]
            code_tokens = [tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] + code_tokens + [
                tokenizer.sep_token]
        else:
    ...
    elif isinstance(tokenizer, CodeGenTokenizer):
        if "func" in js:
            code = " ".join(remove_comments_and_docstrings(js['func'], lang).split())
        else:
            code = " ".join(remove_comments_and_docstrings(js['code'], lang).split())
        ...
        raise ValueError(("Unknown tokenizer type: %s" % type(tokenizer)))
...
def evaluate(args, model, tokenizer, file_name, candidate_file_name, result_file=None):
    ...
    candidate_dataloader = DataLoader(
        candidate_dataset, sampler=candidate_sampler, batch_size=args.eval_batch_size, num_workers=4
    )
    ...
    candidate_lengths = [len(candidate_dataset.examples[i].code_tokens) for i in range(len(candidate_dataset))]
    query_indexs = [query_dataset.examples[i].index for i in range(len(query_dataset))]
    query_lengths = [len(query_dataset.examples[i].code_tokens) for i in range(len(query_dataset))]
    ...
    for i in range(scores.shape[0]):
        ...
        results[query_index] = {}
        results[query_index]["query_length"] = query_lengths[i]
        results[query_index]["reldoc_lengths"] = []
        Avep = []
        for j, index in enumerate(list(sort_ids[i])):
            ...
            if int(candidate_labels[index]) == label:
                results[query_index]["reldoc_lengths"].append(candidate_lengths[index])
...
def main():
    ...
    parser.add_argument("--encoder_state_dict", default=None, type=str,
                        help="The model checkpoint for encoder weights initialization.")
    parser.add_argument("--model_type", default='roberta', type=str, help="Type of model.")
    ...
    parser.add_argument("--result_file", default=None, type=str,
                        help="The output result filepath (a json file).")
    ...
    # build model
    if args.model_type == 'hf_roberta':
        tokenizer = RobertaTokenizer.from_pretrained(args.model_name_or_path)
        model = RobertaModel.from_pretrained(args.model_name_or_path)
    elif args.model_type == 'hf_codegen':
        tokenizer = CodeGenTokenizer.from_pretrained(args.model_name_or_path, pad_token="<pad>")
        tokenizer.max_model_input_sizes['gpt2'] = 1e20
        model = CodeGenModel.from_pretrained(args.model_name_or_path)
    else:
        raise ValueError("Invalid model type: %s" % args.model_type)

    model = Model(model, pad_token_id=tokenizer.pad_token_id)
    model.load_encoder_state_dict(args.encoder_state_dict, logger)
    logger.info("Training/evaluation parameters %s", args)
    logger.info("Que lang: %s", args.query_lang)
    logger.info("Doc lang: %s", args.candidate_lang)
    model.to(args.device)

    if args.n_gpu > 1:
        model = torch.nn.DataParallel(model)

    result = evaluate(args, model, tokenizer, args.query_data_file, args.candidate_data_file, args.result_file)
